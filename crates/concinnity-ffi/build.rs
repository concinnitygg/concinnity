...
//    `concinnity-toolchain` helper. This crate produces the FFI cdylib a host
//    app loads, not a final executable, so it does NOT bundle runtime DLLs or
//    emit the Agility `D3D12SDKVersion`/`D3D12SDKPath` bin exports; those belong
//    to whichever package owns the final .exe (concinnity-cli, concinnity-runtime,
//    the examples). The `-bins` link args would also be rejected here, as there
//    is no bin target. The one link directive kept (via `bundle_dlls: false`) is
//    the NGX import lib: the cdylib links the DLSS code and must resolve its NGX
//    symbols.
//
