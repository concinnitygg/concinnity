// src/ffi.rs
//
// General-purpose C-ABI boundary for embedding the engine in a host
// application: any C-compatible caller can drive it. cbindgen turns this file
// into `include/concinnity.h`, so the function list below is the public
// surface.
//
...
// RUST_LOG. See concinnity_engine::app::run::init_logging.
#[unsafe(no_mangle)]
...
pub extern "C" fn cn_add_with_template(
    ...
    match concinnity_editor::add_to_path(
        &jsonl_path_str,
        name_str.as_deref(),
        &target_str,
...
pub extern "C" fn cn_rm(
    ...
    match concinnity_editor::rm_at_path(&jsonl_path_str, &name_str) {
        Ok(()) => {
...
pub extern "C" fn cn_check_world(world_jsonl_path: *const c_char) -> c_int {
    ...
    match concinnity_editor::check_at_path(&jsonl_path_str) {
        Ok(()) => 1,
        Err(e) => {
...
pub extern "C" fn cn_preview_start(
    ...
    let world = match concinnity_editor::world_from_loaded(loaded) {
        Ok(w) => w,
        Err(e) => {
...
// host process's run-loop observers (e.g. a display-refresh observer) keep
