...
mod tests {
    ...
    fn id_families_are_disjoint() {
        ...
        for id in crate::editor::hud::all_ids() {
            claim(id, "top bar".to_string());
        }
        for id in crate::editor::viewport::highlight::all_sprite_ids() {
            claim(id, "selection highlight".to_string());
        }
        claim(
            crate::editor::viewport::marquee::RECT,
            "marquee rect".to_string(),
        );
        for id in crate::editor::viewport::gizmo::all_sprite_ids() {
            claim(id, "gizmo".to_string());
        }
        for id in crate::editor::viewport::billboards::all_sprite_ids() {
            claim(id, "billboards".to_string());
        }
        for id in crate::editor::viewport::billboards::all_label_ids() {
            claim(id, "billboard glyphs".to_string());
        }
        claim(
            crate::editor::viewport::gizmo::MODE_LABEL,
            "gizmo mode label".to_string(),
        );
        claim(
            crate::editor::viewport::cursor::CURSOR,
            "editor cursor".to_string(),
        );
        for id in crate::editor::create_menu::all_sprite_ids() {
            claim(id, "create menu sprites".to_string());
        }
        for id in crate::editor::create_menu::all_label_ids() {
            claim(id, "create menu labels".to_string());
        }
        for id in crate::editor::view_menu::all_sprite_ids() {
            claim(id, "display menu sprites".to_string());
        }
        for id in crate::editor::view_menu::all_label_ids() {
            claim(id, "display menu labels".to_string());
        }
        for id in crate::editor::toast_overlay::all_sprite_ids() {
            claim(id, "toast sprites".to_string());
        }
        for id in crate::editor::toast_overlay::all_label_ids() {
            claim(id, "toast labels".to_string());
        }
        for id in crate::editor::modal::all_sprite_ids() {
            claim(id, "confirm dialog sprites".to_string());
        }
        for id in crate::editor::modal::all_label_ids() {
            claim(id, "confirm dialog labels".to_string());
        }
        claim(
            crate::editor::worlds::cinematic::FADE,
            "start screen shot fade".to_string(),
        );
        for id in crate::editor::worlds::loading::all_sprite_ids() {
            claim(id, "loading cover".to_string());
        }
        for id in crate::editor::worlds::loading::all_label_ids() {
