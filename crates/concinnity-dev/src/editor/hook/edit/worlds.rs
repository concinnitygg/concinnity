...
impl EditorHook {
    ...
    pub(in crate::editor::hook) fn make_worlds_view<'a>(
        &'a self,
        mouse: [f32; 2],
    ) -> WorldsView<'a> {
        WorldsView {
    ...
    // describes.
    pub(in crate::editor::hook) fn apply_worlds_action(
        &mut self,
        action: WorldsAction,
        world: &mut World,
    ) {
    ...
    pub(in crate::editor::hook) fn apply_worlds_confirm(
        &mut self,
        confirm: WorldsConfirm,
        world: &mut World,
    ) {
        match confirm {
    ...
    pub(in crate::editor::hook) fn prompt_world_name(&mut self, rejected: Option<String>) {
