...
pub(in crate::editor::hook) struct VariablesData {
    pub rows: Vec<Row>,
    pub authoritative: bool,
    pub status: Option<String>,
    // Whether a live session's values are showing in place of the declared
    // starting values (see `hook/drive/trace.rs`).
    pub live: bool,
}
...
impl EditorHook {
    ...
    pub(in crate::editor::hook) fn variables_entry(&self) -> Option<usize> {
        self.entries
            .iter()
            .position(|e| entry_type(e) == Some("Variables"))
    }

    pub(in crate::editor::hook) fn variables_data(&self) -> VariablesData {
    ...
    pub(in crate::editor::hook) fn make_variables_view<'a>(
        &'a self,
    ...
    pub(in crate::editor::hook) fn apply_variables_action(
        &mut self,
    ...
    pub(in crate::editor::hook) fn variables_keys(
        &mut self,
        world: &mut World,
        input: &FrameInput,
    ) {
