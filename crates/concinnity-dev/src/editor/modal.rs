// src/editor/modal.rs
//
// The confirmation dialog: a centered panel with a wrapped message and a row
// of two or three buttons over a translucent dim covering the whole screen.
...
pub(crate) fn hit_button(mx: f32, my: f32, vp: [f32; 2], count: usize) -> Option<usize> {
    let p = panel_rect(vp);
...
mod tests {
    ...
    fn panel_centers_and_clamps_below_the_top_bar() {
        ...
        let tiny = panel_rect([500.0, 120.0]);
    ...
    fn hit_button_resolves_each_button_and_misses_the_rest() {
        ...
        for count in [2, 3] {
            for i in 0..count {
                ...
                assert_eq!(hit_button(r[0] + 2.0, r[1] + 2.0, VP, count), Some(i));
            }
            // The message area, the dimmed screen, and the gap between two
            // buttons all miss.
            assert_eq!(hit_button(p[0] + 2.0, p[1] + 2.0, VP, count), None);
            assert_eq!(hit_button(5.0, 5.0, VP, count), None);
            let r1 = button_rect(p, count, 1);
            assert_eq!(hit_button(r1[0] - 1.0, r1[1] + 2.0, VP, count), None);
        }
    }
    ...
    fn hover_highlights_the_button_under_the_cursor() {
        ...
        let mut world = world_with_elements();
        apply(&mut world, VP, "m", &buttons, [r0[0] + 2.0, r0[1] + 2.0]);
        assert_eq!(sprite(&world, button_bg(0)).tint, theme::HOVER_TINT);
        assert_eq!(sprite(&world, button_bg(1)).tint, DANGER_TINT);

        apply(&mut world, VP, "m", &buttons, [r1[0] + 2.0, r1[1] + 2.0]);
        assert_eq!(sprite(&world, button_bg(0)).tint, theme::BUTTON_TINT);
