...
impl StageIds {
    ...
    fn from_scaffold(scaffold: &StoryScaffold) -> Option<Self> {
        Some(Self {
            screen: (scaffold.screen?).id(),
            ending_screen: (scaffold.ending?).id(),
            bg: scaffold.bg.map(Ref::id),
            left: scaffold.left.map(Ref::id),
            center: scaffold.center.map(Ref::id),
            right: scaffold.right.map(Ref::id),
            dialog_box: scaffold.dialog_box.map(Ref::id),
            name: scaffold.name_label.map(Ref::id),
            text: scaffold.text_label.map(Ref::id),
            option_boxes: scaffold.option_boxes.iter().map(|r| r.id()).collect(),
            options: scaffold.options.iter().map(|r| r.id()).collect(),
            start_label: scaffold.start_label.map(Ref::id),
            quit_label: scaffold.quit_label.map(Ref::id),
            continue_label: scaffold.continue_label.map(Ref::id),
            title_screen_id: scaffold.title.map(Ref::id),
            load_label: scaffold.load_label.map(Ref::id),
            pause_view: scaffold.pause.map(Ref::id),
            settings_screen: scaffold.settings.map(Ref::id),
            settings_label: scaffold.settings_label.map(Ref::id),
            marker: scaffold.advance_marker.map(Ref::id),
            log_label: scaffold.log_label.map(Ref::id),
            auto_label: scaffold.auto_label.map(Ref::id),
            skip_label: scaffold.skip_label.map(Ref::id),
            save_label: scaffold.save_label.map(Ref::id),
            overlay_dim: scaffold.overlay_dim.map(Ref::id),
            backlog_label: scaffold.backlog_label.map(Ref::id),
            slot_title: scaffold.slot_title.map(Ref::id),
            slot_boxes: scaffold.slot_boxes.iter().map(|r| r.id()).collect(),
            slot_labels: scaffold.slot_labels.iter().map(|r| r.id()).collect(),
        })
    }
}
...
impl StorySystem {
    pub(crate) fn new(
        story: Story,
        tree: Option<&concinnity_host::store::paths::StateTree>,
    ) -> Self {
        Self {
            story,
            ids: None,
