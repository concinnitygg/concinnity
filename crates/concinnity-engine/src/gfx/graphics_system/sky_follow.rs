...
// centred on the camera is what makes that true anywhere in a world, rather
...
pub(crate) fn centre_on_camera(ctx: &mut PipelineContext, props: &[Entity], cam_pos: [f32; 3]) {
    for &entity in props {
        if ctx
...
mod tests {
    ...
    fn the_sky_follows_the_camera() {
        ...
        centre_on_camera(&mut world.context(), &[entity], [1.0, 2.0, -300.0]);
    ...
    fn a_camera_that_has_not_moved_leaves_the_column_untouched() {
        ...
        centre_on_camera(&mut world.context(), &[entity], [4.0, 0.0, 0.0]);
    ...
    fn a_prop_with_no_transform_gains_one() {
        let mut world = World::new();
        let entity = world.spawn();
        centre_on_camera(&mut world.context(), &[entity], [0.0, 5.0, 0.0]);
        let ctx = world.context();
        assert_eq!(
