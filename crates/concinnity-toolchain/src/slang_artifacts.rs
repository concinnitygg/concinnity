...
/// and emits the stub lookup behind a cargo warning; an empty `artifacts` emits
/// the same stub silently, for a host that can compile but has nothing to embed.
...
// (names and paths in, source out) for unit testing. No entries means the stub:
// a `match` with only the fallthrough arm fails clippy in the including crate.
...
// The lookup for a build that embeds nothing: no slangc, or no artifacts.
fn stub_lookup_source(lookup: &str) -> String {
    format!(
        "// @generated by concinnity-toolchain::precompile_slang_artifacts (nothing embedded)\n\
