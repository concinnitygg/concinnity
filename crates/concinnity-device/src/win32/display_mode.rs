...
pub(crate) fn enumerate(hwnd: HWND) -> Vec<DisplayMode> {
...
pub(crate) fn current(hwnd: HWND) -> Option<DisplayMode> {
...
// (borderless) fullscreen. Owned by each backend's context; `reconcile` runs
// once per frame from `window::frame_tick` (after the message pump), so a
// mode chosen in any window mode, a later fullscreen entry, and a menu-driven
// exit all converge on the right monitor state. Cheap when nothing changed
// (two field compares).
pub(crate) struct FullscreenDisplayMode {
...
impl FullscreenDisplayMode {
    ...
    pub(crate) fn set_desired(&mut self, mode: DisplayMode) {
    ...
    pub(crate) fn reconcile(&mut self, hwnd: HWND, fullscreen: bool) -> bool {
        if !fullscreen {
            return self.restore();
        }
