...
fn fill_triangle(
    ...
    let min_x = floor(p.iter().map(|v| v[0]).fold(f32::INFINITY, f32::min));
    let max_x = ceil(p.iter().map(|v| v[0]).fold(f32::NEG_INFINITY, f32::max));
    let min_y = floor(p.iter().map(|v| v[1]).fold(f32::INFINITY, f32::min));
    let max_y = ceil(p.iter().map(|v| v[1]).fold(f32::NEG_INFINITY, f32::max));
...
pub fn shade_sphere(size: u32, albedo: [f32; 3], roughness: f32, metallic: f32) -> RasterImage {
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            let normal = [dx, dy, sqrt(1.0 - d2)];
            let diff = dot(normal, light).max(0.0);
            let spec = powf(dot(normal, h).max(0.0), shininess) * (1.0 - rough * 0.6);
