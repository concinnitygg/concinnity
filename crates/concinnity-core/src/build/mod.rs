...
// consts, the built-in shader sources, and the `Platform` / `SourceBacked`
// types (the engine selects a stage's current-platform source at runtime).
// Submodules stay `pub` so both the client runtime and the build crate can
// reach them across the workspace split.
pub mod asset;
pub mod bcn;
pub mod color_lut;
pub mod cubemap;
