//! Source assembly for the single-source engine shaders.
//!
//! Every consumer that compiles one of `src/render/shaders/*.hlsl` assembles the same
...
//!
//! Every program also leads with one `CN_BACKEND_*` define naming the backend
//! it compiles for ([`backend_define`]). The assembler adds it rather than each
//! program table, so a shader branches on the backend the same way everywhere
//! and no table can forget it.
//!
...
///
/// Several come in pairs, because a shader's resource bindings sit between the
/// halves: MAIN_TYPES / PROBE_TYPES / RT_TYPES / RAYMARCH_TYPES declare the
/// records a binding names, and MAIN_SHADING / PROBE_COMMON / RT_TRACE /
/// RAYMARCH_COMMON the code that reads the bound resources.
///
...
pub const FRAGMENTS: &[(&str, &str)] = &[
    ...
    // the transparent producers' environment fallback reads the probe set.
    ("{TRANSPARENT_PROBES}", "transparent_probes.hlsl"),
    ("{PROBE_COMMON}", "probe_common.hlsl"),
    ("{RT_TYPES}", "rt_types.hlsl"),
    ...
    // marker: the forward and transparent passes both read the cluster grid.
    ("{CLUSTER_TYPES}", "cluster_types.hlsl"),
    ("{TEXTURE_SIZE}", "texture_size.hlsl"),
    ("{PARTICLE_TYPES}", "particle_types.hlsl"),
    ...
    ("{RAYMARCH_TYPES}", "raymarch_types.hlsl"),
    ...
    ("{SURFACE_VERTEX}", "surface_vertex_default.hlsl"),
    ("{SURFACE_FRAGMENT}", "surface_fragment_default.hlsl"),
];
...
/// The define every assembled source leads with, naming the backend it compiles
/// for. A shader branches on it wherever the backends bind one declaration at
/// different slots or in a different shape.
pub fn backend_define(platform: Platform) -> &'static str {
    match platform {
        Platform::Metal => "CN_BACKEND_METAL",
        Platform::DirectX => "CN_BACKEND_DIRECTX",
        Platform::Vulkan => "CN_BACKEND_VULKAN",
    }
}

/// The exact source text a program compiles for `platform`, from the embedded
/// shaders alone. `file` names the shader under `src/render/shaders/`.
pub fn assemble(file: &str, platform: Platform, defines: &[(&str, &str)]) -> String {
    assemble_with(file, platform, defines, shaders::embedded)
}
...
mod tests {
    ...
    const M: Platform = Platform::Metal;

    // The assembled text below the backend define every source leads with.
    fn body(src: &str) -> &str {
        src.strip_prefix("#define CN_BACKEND_METAL 1\n")
            .expect("the backend define leads")
    }
    ...
    // Every assembly leads with the one define naming its backend, ahead of the
    // program's own, and no two backends share one.
    #[test]
    fn the_backend_define_leads_the_program_defines() {
        for platform in Platform::ALL {
            let src = assemble_with("x.hlsl", platform, &[("A", "1")], |_| Some("BODY\n"));
            let want = alloc::format!(
                "#define {} 1\n#define A 1\nBODY\n",
                backend_define(platform)
            );
            assert_eq!(src, want);
        }
        let mut names: Vec<&str> = Platform::ALL.into_iter().map(backend_define).collect();
        names.sort_unstable();
        names.dedup();
    ...
    fn the_post_common_marker_is_spliced_into_the_body() {
        let src = assemble_with("x.hlsl", M, &[], |f| {
            (f == "x.hlsl").then_some("A\n{POST_COMMON}\nB\n")
        });
        assert!(!src.contains("{POST_COMMON}"));
        assert!(src.contains("float2 reflection_source_uv("));
        assert!(body(&src).starts_with("A\n") && src.ends_with("\nB\n"));
    }
    ...
    fn the_paired_fragments_splice_records_before_helpers() {
        ...
        let src = assemble_with("x.hlsl", M, &[], |f| (f == "x.hlsl").then_some(body));
        for (marker, _) in FRAGMENTS {
            assert!(!src.contains(marker), "unspliced {marker}");
        }
        assert!(src.find("struct ProbeSet") < src.find("float3 probe_mask_specular("));
        assert!(src.find("struct RtGeomEntry") < src.find("bool rt_trace_reflection("));
    }

    // The transparent probe block carries the probe helpers its environment
    // fallback calls, so its row has to run ahead of theirs.
    #[test]
    fn the_transparent_probes_splice_the_probe_helpers_they_call() {
        let src = assemble_with("x.hlsl", M, &[], |f| {
            (f == "x.hlsl").then_some("{TRANSPARENT_PROBES}\n")
        });
        for (marker, _) in FRAGMENTS {
            assert!(!src.contains(marker), "unspliced {marker}");
        }
        let helper = src.find("bool probe_mask_blend(");
        let caller = src.find("float3 transparent_environment(");
        assert!(helper.is_some() && helper < caller);
    }
    ...
    fn the_resolver_overrides_the_embedded_copy_and_declining_falls_back() {
        let overridden = assemble_with("fog.hlsl", M, &[], |f| {
            (f == "fog.hlsl").then_some("OVERRIDDEN\n")
        });
        assert_eq!(body(&overridden), "OVERRIDDEN\n");
        assert_eq!(
            assemble_with("fog.hlsl", M, &[], |_| None),
            assemble("fog.hlsl", M, &[])
        );
    }
    ...
    fn a_caller_splice_is_not_rescanned_for_table_markers() {
        ...
        assert_eq!(body(&src), "// see {MAIN_TYPES} and {SURFACE_VERTEX}\n\n");
    }
    ...
    fn a_caller_splice_wins_over_a_table_row_for_the_same_marker() {
        ...
        assert!(
            !src.contains("float4 shade(VertexOut v, GpuObjectData od)\n{"),
    ...
    fn both_passes_take_the_light_records_from_one_fragment() {
        for file in ["main_bindless.hlsl", "raymarch.hlsl"] {
            let src = assemble(file, M, &[]);
