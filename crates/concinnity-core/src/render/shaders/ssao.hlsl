...
// by a define so each variant declares exactly the resources it binds:
//
    ...
    float4 pixel : SV_Position) : SV_Target
{
    ...
    for (int s = 0; s < SSAO_SLICES; s++)
    {
        ...
        for (int step = 1; step <= SSAO_STEPS; step++)
        {
            ...
            float2 uvp = uv + off;
            float dp = gbuffer.Sample(gbuffer_samp, uvp).a;
            if (dp > 0.0)
            {
            ...
            float2 uvm = uv - off;
            float dm = gbuffer.Sample(gbuffer_samp, uvm).a;
            if (dm > 0.0)
            {
    ...
    for (int y = -2; y <= 2; y++)
    {
        for (int x = -2; x <= 2; x++)
        {
            ...
            float w = (d > 0.0)
                ...
                : 0.0;
            sum  += ao_raw.Sample(ao_raw_samp, tap).r * w;
            wsum += w;
        }
    }
    return (wsum > 1e-4) ? (sum / wsum) : ao_raw.Sample(ao_raw_samp, uv).r;
}

#else
#error "ssao.hlsl: define SSAO_KERNEL or SSAO_BLUR"
#endif
