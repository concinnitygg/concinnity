...
// The two branches carry the same Vulkan bindings and differ only in the
// register numbers, because Metal has one buffer index space where DirectX has
// three: a `t0` and a `u0` that never collide on DXIL would both land on
// buffer(0). The shared branch's numbers are the Metal indices its encoder
// already binds (see concinnity-shader's `metal_bindings`); the DirectX one is
// the root signature in `directx/raytrace.rs`, which puts the params in root
// constants at b0.
...
ConstantBuffer<SkinParams> params : register(b0);
#else
[[vk::binding(0, 0)]] ByteAddressBuffer src : register(t0);
