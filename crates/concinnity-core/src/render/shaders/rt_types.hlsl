...
// its RT_TYPES marker (see shader_source.rs). Declares no resource and no entry
...
struct RtParams
{
    ...
    // unshadowed.
    float sun_shadows;
    // Per-axis divisor of the reduced glass reflection pre-pass; 1.0 when glass
    // traces its reflection in place.
    float trace_divisor;
    float _pad2;
