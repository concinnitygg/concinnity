...
//
// Metal takes its index from each resource's `register()` number, so the init
// and downsample kernels read their source at texture(0) and write at
// texture(1) with the params at buffer(0), which is what `metal/hiz.rs` binds.
...
//
// The 2x2 patch is what keeps the base-level read fast: neighboring lanes
// address neighboring texels, and the tile stays small enough that a frame
...
[[vk::push_constant]]
ConstantBuffer<HizSpdParams> params : register(b0);
...
[[vk::push_constant]]
ConstantBuffer<HizParams> params : register(b0);
...
[[vk::binding(0, 0)]]
Texture2DMS<float> src_depth : register(t0);
...
[[vk::binding(0, 0)]] [[vk::image_format("r32f")]]
RWTexture2D<float> src_hiz : register(u0);
...
#else
#error "hiz_build.hlsl: define HIZ_INIT_MSAA, HIZ_INIT_SINGLE, HIZ_DOWNSAMPLE, HIZ_SPD_MSAA, HIZ_SPD_SINGLE, or HIZ_SPD_TAIL"
#endif

#if defined(HIZ_INIT_MSAA) || defined(HIZ_INIT_SINGLE) || defined(HIZ_DOWNSAMPLE)

[[vk::binding(1, 0)]] [[vk::image_format("r32f")]]
RWTexture2D<float> dst_mip : register(u1);
...
[[vk::binding(HIZ_SPD_MIP_BINDING, 0)]] [[vk::image_format("r32f")]]
RWTexture2D<float> spd_mips[HIZ_SPD_LEVELS] : register(u0);

#endif

#if defined(HIZ_INIT_MSAA) || defined(HIZ_SPD_MSAA)

// `Texture2DMS<float>::Load(int2, int)` and `Texture2D<float>::Load(int3)` are
// different overloads, so the multisample variants read a sample through this
// and the single-sample ones call `Load` directly.
