...
// camera VP, jittered when TAA is on so the rasterized pixel grid matches the
// main pass exactly. The fragment stage reconstructs the world-space sample
// point of each rasterized pixel from the main pass's depth attachment,
...
// setBytes at the buffer indices the register numbers name, which are also the
// D3D root signature's slots.
[[vk::binding(0, 0)]] ConstantBuffer<DecalView> view : register(b0);
[[vk::binding(1, 0)]] ConstantBuffer<DecalParams> params : register(b1);

// The scene depth at 0 and this decal's albedo at 1, on every backend. The
// albedo and its sampler live in their own Vulkan set so swapping decals
// rebinds set 1 rather than the per-frame set.
#if USE_MSAA
[[vk::binding(2, 0)]] Texture2DMS<float> scene_depth : register(t0);
#else
[[vk::binding(2, 0)]] Texture2D<float> scene_depth : register(t0);
#endif
[[vk::binding(0, 1)]] Texture2D<float4> decal_tex : register(t1);
[[vk::binding(1, 1)]] SamplerState decal_tex_sampler : register(s0);
...
// The cube is only a rasterization proxy, so the fragment derives everything it
...
{
    // The single-sample depth resolve shares the rasterized pixel grid with the
    ...
    uv.y = 1.0 - uv.y;
    float4 tex = decal_tex.Sample(decal_tex_sampler, uv);
