...
// The register numbers are the Metal indices (see concinnity-shader's
// `metal_bindings`) and the D3D root signature's slots alike.
...
// A prefix of `PostProcessParams` in render_types.rs; the prefilter reads only
// the threshold, knee and exposure.
...
[[vk::push_constant]] ConstantBuffer<PostProcessParams> post : register(b0);
#endif
...
{
    float3 a = src.Sample(src_sampler, uv + texel * float2(-2.0, -2.0)).rgb;
    float3 b = src.Sample(src_sampler, uv + texel * float2( 0.0, -2.0)).rgb;
    float3 c = src.Sample(src_sampler, uv + texel * float2( 2.0, -2.0)).rgb;
    float3 d = src.Sample(src_sampler, uv + texel * float2(-2.0,  0.0)).rgb;
    float3 e = src.Sample(src_sampler, uv + texel * float2( 0.0,  0.0)).rgb;
    float3 f = src.Sample(src_sampler, uv + texel * float2( 2.0,  0.0)).rgb;
    float3 g = src.Sample(src_sampler, uv + texel * float2(-2.0,  2.0)).rgb;
    float3 h = src.Sample(src_sampler, uv + texel * float2( 0.0,  2.0)).rgb;
    float3 i = src.Sample(src_sampler, uv + texel * float2( 2.0,  2.0)).rgb;
    float3 j = src.Sample(src_sampler, uv + texel * float2(-1.0, -1.0)).rgb;
    float3 k = src.Sample(src_sampler, uv + texel * float2( 1.0, -1.0)).rgb;
    float3 l = src.Sample(src_sampler, uv + texel * float2(-1.0,  1.0)).rgb;
    float3 m = src.Sample(src_sampler, uv + texel * float2( 1.0,  1.0)).rgb;
...
{
    float2 texel = 1.0 / texture_size(src);
...
float3 upsample_tent(float2 uv, float2 texel)
{
    ...
    sum += src.Sample(src_sampler, uv + texel * float2(-1.0, -1.0)).rgb * 1.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 0.0, -1.0)).rgb * 2.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 1.0, -1.0)).rgb * 1.0;
    sum += src.Sample(src_sampler, uv + texel * float2(-1.0,  0.0)).rgb * 2.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 0.0,  0.0)).rgb * 4.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 1.0,  0.0)).rgb * 2.0;
    sum += src.Sample(src_sampler, uv + texel * float2(-1.0,  1.0)).rgb * 1.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 0.0,  1.0)).rgb * 2.0;
    sum += src.Sample(src_sampler, uv + texel * float2( 1.0,  1.0)).rgb * 1.0;
...
#else
#error "bloom.hlsl: define BLOOM_PREFILTER, BLOOM_DOWNSAMPLE, or BLOOM_UPSAMPLE"
#endif
