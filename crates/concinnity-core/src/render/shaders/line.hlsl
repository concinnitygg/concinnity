...
// sit off the line center (gfx::lines::build_vertices), so the vertex stage
// only applies the camera VP.
//
// Alpha does the two things a line needs: `edge` fades the outer pixel of the
// ribbon so the line antialiases without MSAA, and the vertex color's own
...
//
// Both resources take their Metal index from the number on their `register()`
// (see concinnity-shader's `metal_bindings`), and D3D and Metal agree on both, so
// one annotation serves the pair: the view at buffer(0) / b0 and the scene
// depth at texture(0) / t0.
...
// Matches `LineVertex` (32 B: pos at 0, edge at 12, color at 16), which the
