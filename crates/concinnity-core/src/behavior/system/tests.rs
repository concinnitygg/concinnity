...
// A store two runs can share, so a save written by one is what the next
// restores from. Cloneable so a test can look at what was written.
...
fn worlds_without_a_save_node_never_read_state() {
    ...
    // Same store, but nothing saves: the world starts fresh.
...
// A world with no store at all runs its behaviors and keeps nothing: the
// `save` node is a no-op rather than a failure.
#[test]
