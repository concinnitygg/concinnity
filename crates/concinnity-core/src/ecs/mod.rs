...
//! system types, so it is written in the client crate, whose own `ecs` module
//! holds that table and the resources only a renderer-bearing runtime has.
//!
...
// Per-frame facilities carried on `PipelineContext`.
...
// here for the physics / audio subsystem crates; the client's `define_systems!`
...
// (invoked in `registry`). Re-exported here so the rest of the crate reaches
// them as `ecs::*`. The authoring `RegisteredType` registry
