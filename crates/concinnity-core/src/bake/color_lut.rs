...
mod tests {
    ...
    // rather than parsed as a 3D cube with a wrong entry count.
    #[test]
    fn parse_cube_rejects_a_one_dimensional_lut() {
        let err = parse_cube("LUT_1D_SIZE 16\n0 0 0\n").expect_err("1D is unsupported");
        assert!(err.contains("1D"), "{err}");
        assert!(err.contains("LUT_3D_SIZE"), "{err}");
    }
