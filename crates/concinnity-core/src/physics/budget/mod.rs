...
pub use derive::{PhysicsBudget, PhysicsCounts};

use alloc::collections::BTreeSet;

use crate::memory::{Ledger, MemTag, Realm};
use crate::physics::{BodyHandle, ContactHit, SensorCrossing, Simulation};
...
/// [`PhysicsSystem`](super::PhysicsSystem)'s init goes on to build.
///
...
// Bytes a `Vec<T>` of `capacity` holds. Every container the driver reserves is
// one of these, including the lookup indices (see `super::index`), so the sum
// below is exact rather than a table's load-factor estimate.
