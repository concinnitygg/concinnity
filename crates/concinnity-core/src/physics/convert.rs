// Conversions from the authored representation to the simulation
    ...
    fn joint_spec_converts_revolute_units_to_radians() {
        ...
        match joint_spec(&j) {
            JointSpec::Revolute {
                ...
                assert!((lim[0] - (-core::f32::consts::FRAC_PI_2)).abs() < 1.0e-5);
                assert!((lim[1] - core::f32::consts::FRAC_PI_2).abs() < 1.0e-5);
                let m = motor.expect("motor set");
                assert!((m.target_velocity - core::f32::consts::PI).abs() < 1.0e-5);
