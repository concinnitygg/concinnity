...
pub(crate) fn cast(
    ...
    let impact = match *shape {
        ColliderShape::Ball { radius } => sphere(local, radius.abs()),
        ...
        ColliderShape::Capsule {
            half_height,
            radius,
        } => capsule(local, half_height.abs(), radius.abs()),
    }?;
...
fn capsule(ray: Ray, half_height: f32, radius: f32) -> Option<RayImpact> {
    ...
    if a > PARALLEL {
        ...
        if discriminant >= 0.0 {
            let distance = (-b - sqrt(discriminant)) / a;
...
mod tests {
    ...
    fn the_bounds_filter_never_rejects_a_ray_that_hits() {
        ...
        for i in 0..64 {
            ...
            let origin = vec3(cos(angle) * 6.0, sin(angle) * 6.0, -4.0);
