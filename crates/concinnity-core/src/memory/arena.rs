...
impl Arena {
    ...
    pub fn tagged(bytes: usize, tag: MemTag) -> Self {
        crate::memory::ledger().add(tag, Realm::Host, bytes as u64);
...
impl Drop for Arena {
    fn drop(&mut self) {
        if let Some(tag) = self.tag {
            crate::memory::ledger().release(tag, Realm::Host, self.cap as u64);
        }
        if self.cap == 0 {
            return;
        }
...
mod tests {
    ...
    fn a_tagged_arena_reports_its_reservation_for_as_long_as_it_lives() {
        ...
        let held = || {
            crate::memory::ledger()
                .usage(MemTag::Scratch, Realm::Host)
                .bytes
        };
    ...
    fn an_untagged_arena_reports_nothing() {
        let held = || crate::memory::ledger().usage(MemTag::Ui, Realm::Host).bytes;
        let before = held();
