// concinnity-physics/src/sim/collide/box_box.rs
//
...
use crate::sim::contact::{MAX_MANIFOLD_POINTS, Manifold, ManifoldPoint};
...
fn separating_axis(a: OrientedBox, b: OrientedBox, margin: f32) -> Option<Query> {
    ...
    let mut consider = |axis: Vec3, winner: Winner, preference: f32| -> bool {
        ...
        let separation =
            libm::fabsf(projection) - (a.extent_along(normal) + b.extent_along(normal));
