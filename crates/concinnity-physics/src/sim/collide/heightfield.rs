// concinnity-physics/src/sim/collide/heightfield.rs
//
...
impl Heightfield {
    ...
    pub(crate) fn new(
        ...
        let (width, depth) = (libm::fabsf(scale.x), libm::fabsf(scale.z));
    ...
    pub(crate) fn cell_at(&self, point: Vec3) -> (usize, usize) {
        ...
        let clamp = |value: f32, start: f32, step: f32, count: usize| {
            libm::floorf((value - start) / step).clamp(0.0, (count - 1) as f32) as usize
        };
        (
    ...
    pub(crate) fn cell_range(&self, bounds: Aabb) -> Option<CellRange> {
        ...
        let span = |low: f32, high: f32, start: f32, step: f32, count: usize| {
            let first = libm::floorf((low - start) / step);
            let last = libm::floorf((high - start) / step);
