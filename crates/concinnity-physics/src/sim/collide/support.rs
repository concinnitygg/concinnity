// concinnity-physics/src/sim/collide/support.rs
//
...
impl OrientedBox {
    ...
    pub(crate) fn extent_along(&self, direction: Vec3) -> f32 {
        libm::fabsf(direction.dot(self.pose.axis(0))) * self.half.x
            + libm::fabsf(direction.dot(self.pose.axis(1))) * self.half.y
            + libm::fabsf(direction.dot(self.pose.axis(2))) * self.half.z
    }
}
        ...
        for sign in [1.0f32, -1.0] {
            ...
            for i in 0..in_len {
                ...
                if (dc >= 0.0) != (dn >= 0.0) && out_len < MAX_CLIP_POINTS {
                    let denom = dc - dn;
                    let t = if libm::fabsf(denom) > f32::MIN_POSITIVE {
                        dc / denom
                    } else {
                        0.0
                    };
...
pub(crate) fn clipped_feature(point: Vec3, corners: &[Vec3; 4], half: Vec3, axis: usize) -> u32 {
    ...
    for (bit, (clip_axis, sign)) in [(u, 1.0f32), (u, -1.0), (v, 1.0), (v, -1.0)]
        ...
        .enumerate()
    {
        if libm::fabsf(sign * point.get(clip_axis) - half.get(clip_axis)) <= FEATURE_EPSILON {
            sides |= 1 << bit;
        }
    }
...
mod tests {
    ...
    fn a_face_larger_than_the_reference_clips_down_to_it() {
        ...
        for p in &clipped.points[..clipped.count] {
            assert!(
                libm::fabsf(p.x) <= 1.0 + 1.0e-5 && libm::fabsf(p.z) <= 1.0 + 1.0e-5,
                "{p:?}"
            );
        }
    }
    ...
    fn reduction_keeps_the_deepest_point_and_spreads_the_rest() {
        ...
        for i in 0..8 {
            ...
            points[i] = vec3(libm::cosf(angle), 0.0, libm::sinf(angle));
            seps[i] = -0.01;
        }
        seps[5] = -0.5;
        let mut keep = [0usize; 4];
