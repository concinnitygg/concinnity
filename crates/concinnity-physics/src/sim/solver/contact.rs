// concinnity-physics/src/sim/solver/contact.rs
//
...
use crate::sim::contact::{MAX_MANIFOLD_POINTS, Manifold};
use crate::sim::coupling::Coupling;
...
pub(crate) fn solve(
    ...
    for constraint in constraints.iter_mut() {
        ...
        for point in &mut constraint.points[..count] {
            ...
            let magnitude = libm::sqrtf(wanted[0] * wanted[0] + wanted[1] * wanted[1]);
            if magnitude > limit {
