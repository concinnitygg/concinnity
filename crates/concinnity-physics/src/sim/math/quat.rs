// concinnity-physics/src/sim/math/quat.rs
//
...
impl Quat {
    ...
    pub(crate) fn from_euler_deg(euler_deg: [f32; 3]) -> Self {
        ...
        Self::from_axis_angle(Vec3::Y, yaw * to_rad)
            .mul(Self::from_axis_angle(Vec3::X, pitch * to_rad))
            .mul(Self::from_axis_angle(Vec3::Z, roll * to_rad))
    }

    /// Decompose back into engine Euler degrees `[pitch, yaw, roll]`.
    pub(crate) fn to_euler_deg(self) -> [f32; 3] {
        let q = self.normalize();
        // The rotation matrix entries the YXZ decomposition needs.
        let m12 = 2.0 * (q.y * q.z - q.w * q.x);
        let sin_pitch = (-m12).clamp(-1.0, 1.0);
        let pitch = libm::asinf(sin_pitch);
        let to_deg = 180.0 / core::f32::consts::PI;
        // Near +-90 degrees of pitch, yaw and roll fold into one angle; pin
        // roll to zero and put the whole rotation in yaw.
        if libm::fabsf(sin_pitch) > 0.999_999 {
            let m20 = 2.0 * (q.x * q.z - q.w * q.y);
            let m00 = 1.0 - 2.0 * (q.y * q.y + q.z * q.z);
            return [pitch * to_deg, libm::atan2f(-m20, m00) * to_deg, 0.0];
        }
        let m10 = 2.0 * (q.x * q.y + q.w * q.z);
        let m11 = 1.0 - 2.0 * (q.x * q.x + q.z * q.z);
        let m02 = 2.0 * (q.x * q.z + q.w * q.y);
        let m22 = 1.0 - 2.0 * (q.x * q.x + q.y * q.y);
        [
            pitch * to_deg,
            libm::atan2f(m02, m22) * to_deg,
            libm::atan2f(m10, m11) * to_deg,
        ]
    }
