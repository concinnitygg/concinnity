// concinnity-physics/src/sim/math/vec3.rs
//
...
impl Vec3 {
    ...
    pub(crate) fn any_perpendicular(self) -> Self {
        if libm::fabsf(self.x) <= libm::fabsf(self.y) && libm::fabsf(self.x) <= libm::fabsf(self.z)
        {
            self.cross(Self::X).normalize_or(Self::Y)
        } else if libm::fabsf(self.y) <= libm::fabsf(self.z) {
...
mod tests {
    ...
    fn a_perpendicular_is_perpendicular_and_unit_length_for_every_axis() {
        for dir in [
            ...
            assert!(libm::fabsf(p.dot(dir)) < 1.0e-6, "{dir:?} -> {p:?}");
            assert!(libm::fabsf(p.length() - 1.0) < 1.0e-6, "{dir:?} -> {p:?}");
        }
    }
