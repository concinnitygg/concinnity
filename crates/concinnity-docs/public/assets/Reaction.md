...
- `on`: An object. The event that fires this reaction: `"start"` (world start), `{"timer": {"interval": seconds, "repeat": bool}}`, `{"variable": "name"}` (the named variable changed value), `{"enter": "volume"}` / `{"exit": "volume"}` (something crossed the named [TriggerVolume](TriggerVolume.md)), or `{"interact": "entity"}` (the interact key pressed on the named entity).
