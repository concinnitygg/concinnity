...
//! `gltf_source`, `wavefront`). `panorama` recognizes the one `.glb` shape that
...
// Neutral gray vertex color for imported geometry, so a mesh takes its
...
/// Recognizes a `.glb` that packages an environment image as a sphere you stand
