...
use concinnity_core::bake::texture::{TextureFormat, TextureImage, TextureMip};
...
fn compile_block_format(
    ...
    if decodes {
        let rgba = crate::codec::bcn::decode(tex_format, &level_blocks[0], width, height)?;
        tracing::info!(
...
pub(crate) fn decode_ktx2_rgba8(bytes: &[u8]) -> Result<(u32, u32, Vec<u8>), String> {
    ...
    match image.format {
        ...
        other => Ok((
            w,
            h,
            crate::codec::bcn::decode(other, &image.mips[0].data, w, h)?,
        )),
    }
}
...
// Synthetic KTX2 containers for this crate's tests: `compile::texture` builds them to
...
mod tests {
    ...
    fn decode_blocks_to_rgba8_has_no_bc7_decoder() {
        let err = crate::codec::bcn::decode(TextureFormat::Bc7, &[0u8; 16], 4, 4).unwrap_err();
    ...
    fn ktx2_psnr_etc1s_stays_above_quality_floor() {
        ...
        let (sw, sh, source) =
            crate::compile::texture::decode_source(png, 0, None).expect("decode source png");
        assert_eq!((sw, sh), (256, 256));

        let image = compile_ktx2(&fixture("wood_256_etc1s.ktx2")).expect("etc1s compile");
        assert_eq!(image.format, TextureFormat::Bc1);
        let decoded =
            crate::codec::bcn::decode_bc1(&image.mips[0].data, 256, 256).expect("bc1 decode");
