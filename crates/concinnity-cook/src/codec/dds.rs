...
use concinnity_core::bake::texture::{MAX_MIP_LEVELS, TextureFormat, TextureMip};
...
pub(crate) fn decode_dds(bytes: &[u8]) -> Result<(u32, u32, Vec<u8>), String> {
    ...
    let pixels = crate::codec::bcn::decode(format, &bytes[PIXELDATA_OFFSET..], width, height)?;
...
// Synthetic legacy DDS containers for this crate's tests: `compile::texture` builds
