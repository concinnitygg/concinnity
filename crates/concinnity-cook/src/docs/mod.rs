...
//!
//! `build/docs.rs` reads the rustdoc off every authorable asset's schema struct,
...
// enums) they embed.
#[cfg(test)]
pub(crate) fn describe(type_name: &str) -> Option<&'static AssetDoc> {
    ASSET_DOCS
        .iter()
...
// not a whole page.
#[cfg(test)]
pub(crate) fn summary(type_name: &str) -> Option<&'static str> {
