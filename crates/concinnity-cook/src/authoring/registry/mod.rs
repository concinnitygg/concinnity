...
macro_rules! __meta_validate {
    ($val:expr;) => { $val };
    ($val:expr; validate: $f:ident $($r:tt)*) => {
...
/// The world entry that declares one authored asset under the `$id` `id`, as
/// a `{"type", "args"}` object. The caller never names a JSON type.
pub fn asset_entry<T: Authored>(id: &str, value: &T) -> std::io::Result<serde_json::Value> {
    let args = serde_json::to_value(value)
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))?;
    let mut entry = serde_json::json!({ "type": T::TYPE, "args": args });
    crate::authoring::world::set_entry_id(&mut entry, id);
    Ok(entry)
}

/// Write a reference into an asset entry: `field` is set to the asset name
...
mod authored_tests {
    ...
    fn set_reference_names_a_field_the_typed_value_cannot_carry() {
        let mut entry = asset_entry("hero_shape", &CharacterShape::default()).expect("serializes");
        set_reference(&mut entry, "target", "hero").expect("patched");
        assert_eq!(entry["args"]["target"], "hero");
        assert_eq!(entry["args"]["$id"], "hero_shape");
        assert_eq!(entry["type"], "CharacterShape");
