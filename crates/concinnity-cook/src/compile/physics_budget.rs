...
fn spawn_reservation(assets: &[WorldJsonlAsset]) -> SpawnReservation {
    let sources = crate::check::physics::collider_spawn_sources(assets);
    let mut out = SpawnReservation::default();
    for spawner in &sources.spawners {
        match crate::compile::spawn_population::population(spawner.interval, spawner.lifetime) {
...
mod tests {
    ...
    // A world with physics content that declares no PhysicsConfig receives one
    // at start, carrying the engine defaults. The budget is derived here from
    // the expanded list, which has none, so the two must agree: adding the
    // config the world start injects must move nothing.
    #[test]
    fn the_config_injected_at_start_leaves_the_budget_and_the_warning_alone() {
        ...
        // The same world plus the config the world start injects.
        let mut with = expanded.clone();
        with.push(physics_config(
            serde_json::to_value(concinnity_core::components::PhysicsConfig::default())
                .expect("the engine defaults serialize"),
        ));
        assert_eq!(count(&expanded), count(&with));
        assert_eq!(compute(&expanded), compute(&with));
        assert_eq!(spawn_headroom(&expanded), 0);
        assert_eq!(spawn_headroom(&with), 0);
