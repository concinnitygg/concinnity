...
mod tests {
    ...
    // beside it, mirroring crate::glb::test_fixtures::static_triangle_*.
    ...
    fn parse_file_resolves_an_external_buffer() {
        let dir = tempfile::tempdir().unwrap();
        let bin = crate::glb::test_fixtures::static_triangle_bin();
    ...
    fn from_slice_leaves_a_missing_glb_chunk_unresolved() {
        let glb = crate::glb::test_fixtures::make_glb(
            &crate::glb::test_fixtures::static_triangle_json(),
            None,
        );
        let doc = GltfDoc::from_slice(&glb, None, "t.glb").expect("parse");
        let buffer = doc.doc.document.buffers().next().unwrap();
