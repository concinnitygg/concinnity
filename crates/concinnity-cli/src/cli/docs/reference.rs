...
//
// The asset schema is split across two source trees: the plain data schema
// (structs, their rustdoc, `Default`s, and enums) in concinnity-asset, and the
// `impl Component` blocks in concinnity-core. Both are read here and joined into
// one index, so a struct and its impl rejoin by name.
//
...
// The schema source trees, relative to a checkout of the engine.
const AUTHORED_SCHEMA: &str = "crates/concinnity-asset/src";
...
pub(super) fn build(engine_root: &Path) -> io::Result<Vec<AssetDoc>> {
    let authored_src = engine_root.join(AUTHORED_SCHEMA);
    ...
    // `doc_model` was the vocabulary the old baked tables were written in; the
    // authored tree is otherwise taken whole.
