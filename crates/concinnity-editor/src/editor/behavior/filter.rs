...
//
// Which of a row's options a typed query keeps, and in what order. The point of
// typing is that the first answer is the one wanted, so matches are ranked
// rather than merely collected: a verb the query starts is offered before a verb
// that merely contains it, and before an option whose hint mentions it at all.
//
// Underscores are dropped from both sides of a verb comparison, so "foreach"
// reaches `for_each` without the author having to remember where the engine puts
// its separators. Hints are sentences, so those match as written.
...
// keeps every option in the vocabulary's own order.
pub(crate) fn matching(picks: &[Pick], query: &str) -> Vec<usize> {
    let typed = query.trim();
    let verb_query = squashed(typed);
    let hint_query = typed.to_lowercase();
    let mut hits: Vec<(u8, usize)> = picks
        .iter()
        .enumerate()
        .filter_map(|(i, p)| rank(p, &verb_query, &hint_query).map(|r| (r, i)))
        .collect();
    // The index is the tiebreak, so options of equal rank keep the order the
    // vocabulary declares them in.
    hits.sort_by_key(|&(rank, i)| (rank, i));
    hits.into_iter().map(|(_, i)| i).collect()
}

// How well `pick` answers the query, lower being better, `None` for no match.
fn rank(pick: &Pick, verb_query: &str, hint_query: &str) -> Option<u8> {
    // The caption, not the raw verb: the clear-to-unset entry draws as a word it
    // does not carry, and matching what is on screen is what the author expects.
    let verb = squashed(edit::pick_caption(pick));
    if verb.starts_with(verb_query) {
        return Some(STARTS);
    }
    if verb.contains(verb_query) {
        return Some(CONTAINS);
    }
    let hint = pick.hint.to_lowercase();
