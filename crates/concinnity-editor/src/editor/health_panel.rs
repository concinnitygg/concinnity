...
// The model (and what those three numbers mean per row) is `health.rs`; this
// module only places elements.
//
...
use super::health::{self, HealthSnapshot, Meter};
...
pub(crate) fn size() -> [f32; 2] {
    [
        PANEL_W,
        widget::TITLE_H + ROWS as f32 * ROW_H + CHURN_H + BOTTOM_PAD,
    ]
}
...
pub(crate) fn hit_test(mx: f32, my: f32, o: [f32; 2]) -> bool {
    point_in(mx, my, panel_rect(o))
}
...
pub(crate) fn apply(world: &mut World, snap: &HealthSnapshot, o: [f32; 2], mouse: [f32; 2]) {
    ...
    match health::churn_text(snap) {
        Some(text) => {
            let y = o[1] + widget::TITLE_H + ROWS as f32 * ROW_H;
            place_text(
                world,
                CHURN_LABEL,
...
pub(crate) fn all_label_ids() -> Vec<AssetId> {
    let mut ids = vec![TITLE_LABEL, CLOSE_LABEL, CHURN_LABEL];
    for i in 0..ROWS {
...
mod tests {
    ...
    fn hit_test_swallows_panel_presses_and_lets_misses_through() {
        let o = [100.0, 50.0];
        assert!(hit_test(o[0] + 10.0, o[1] + widget::TITLE_H + 10.0, o));
        assert!(!hit_test(o[0] - 5.0, o[1], o));
        assert!(!hit_test(o[0] + PANEL_W + 1.0, o[1] + 10.0, o));
    }
