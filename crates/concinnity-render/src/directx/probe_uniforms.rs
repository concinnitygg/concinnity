...
//
// GPU-free (plain repr(C) types, no D3D12), so it lives in concinnity-render
// and its layout tests count toward coverage; the DirectX backend re-exports it
// under `crate::directx::probe_uniforms`.
//
