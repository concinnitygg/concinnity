...
// mirrored in the MSL shaders, the coordinate math, and the shader-layout
// binding contract. None of these touch a Metal device or encoder, and they are
...
// re-exports them under `metal` so it keeps its existing `math` / `uniforms` /
// `shader_layout` paths.
