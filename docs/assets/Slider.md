...
- `setting`: A string. Engine setting this row controls (e.g. `"exposure"`). Must be a setting the runtime recognizes as a slider; an unknown key renders but does nothing on drag. Defaults to `""`.
