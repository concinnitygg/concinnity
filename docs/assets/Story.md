...
- `title`: A string. The story title, as shown on the generated title screen. Defaults to `""`.
- `nodes`: An array of [StoryNode](StoryNode.md) objects. The node graph in document order. Play starts at the first node; a node whose last page has no jump and no choices falls through to the next node, and the last node ends the story. Defaults to `[]`.
...
- `save_key`: A string. Stable key naming this story's save file (position + flags, auto-saved page by page under the project data directory). Empty disables saving. Defaults to `""`.
