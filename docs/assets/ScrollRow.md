...
- `elements`: An array of strings. The [Sprite](Sprite.md)/[TextLabel](TextLabel.md) ids that make up this row and move (and clip) together. Click regions are matched to their row by position, so they are not listed here. Defaults to `[]`.
