...
- `tint`: An array of 4 floats. RGBA color the rectangle is filled with, each channel in [0, 1]. Defaults to `[1.0, 1.0, 1.0, 1.0]`.
...
- `screen`: A string. [Screen](Screen.md) this sprite belongs to. `None` means the sprite is always visible (e.g. a scene background). Optional.
- `fit`: A string (see [SpriteFit](SpriteFit.md)). How a screen-owned sprite maps from the reference canvas to the window when their aspect ratios differ. Defaults to `"fit"`.
