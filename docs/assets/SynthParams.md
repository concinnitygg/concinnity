...
- `along`: A float. `bulge`: center along the bone, as a fraction of its length. Defaults to `0.5`.
...
- `source`: A string. `mirror` / `blend_mask`: the authored target to derive from. Defaults to `""`.
