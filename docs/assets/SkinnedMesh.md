...
- `capsule`: A [CharacterCapsule](CharacterCapsule.md) object. Optional character capsule. When set, the mesh collides with the scene as a kinematic character and is moved by the root motion of its [Animation](Animation.md) clips (those with `root_motion` set): the capsule slides along obstacles and settles under gravity, and the rendered mesh follows it. The capsule stands on the mesh origin (its feet), centered `half_height + radius` above it.
