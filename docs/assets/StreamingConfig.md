...
- `texture_budget`: An integer. Maximum number of textures whose load is started per frame, applied independently to the color and normal-map pools. A low value spreads the cost over more frames. Defaults to `4`.
- `texture_cap`: An integer. Maximum number of textures kept loaded at once, applied independently to the color and normal-map pools. When exceeded, the farthest-from-camera textures are dropped. Defaults to `96`.
