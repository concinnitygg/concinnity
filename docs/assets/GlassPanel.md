...
- `normal`: An array of 3 floats. Facing direction of the panel. Normalized on load; defaults to +Z when degenerate.
- `half_size`: An array of 2 floats. Half-width and half-height of the panel, in world units. Defaults to `[1.0, 1.0]`.
- `tint`: An array of 3 floats. Linear-space RGB color the glass tints the scene behind it. Defaults to `[0.7, 0.85, 0.95]`.
