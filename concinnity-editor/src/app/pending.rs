...
pub(crate) mod test_support {
    ...
    pub(crate) fn lock() -> std::sync::MutexGuard<'static, ()> {
        // Route tracing output through the harness's per-test capture so
        // expected ERROR logs stay hidden on pass but replay on failure.
        // First install wins, so cn_init's real subscriber becomes a no-op.
