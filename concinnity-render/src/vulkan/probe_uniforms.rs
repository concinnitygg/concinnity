...
//
// GPU-free (plain repr(C) types, no ash/vk), so it lives in concinnity-render
// and its layout tests count toward coverage; the Vulkan backend re-exports it
// under `crate::vulkan::probe_uniforms`.
//
